...
async function initializeXmtpClient() {
  ...
  console.log("Debug - ENCRYPTION_KEY:", ENCRYPTION_KEY);
...
async function initializeAgent(
  ...
  try {
    ...
    if (!OPENAI_API_KEY?.startsWith('sk-or-')) {
      throw new Error(
        'Invalid OpenRouter API key format. Please use an OpenRouter API key that starts with "sk-or-". ' +
        'You can get one from https://openrouter.ai/keys'
      );
    }

    const llm = new ChatOpenAI({
      modelName: "openai/gpt-3.5-turbo",  // OpenRouter model identifier
      ...
      configuration: {
        ...
        defaultHeaders: {
          "HTTP-Referer": "https://github.com/ephemeraHQ/xmtp-agent-examples", // Required for OpenRouter
          "X-Title": "XMTP Agent Examples" // Optional, but good practice
        }
      }
    });
    ...
    try {
      ...
      console.log("Debug - OpenRouter connection successful");
    } catch (error: any) {
      console.error("OpenRouter Connection Error:", {
        error: error.message,
        type: error.type,
        status: error.status
      });
      if (error.message.includes('API key')) {
        throw new Error(
          'OpenRouter API key authentication failed. Please make sure you are using a valid API key from https://openrouter.ai/keys'
        );
      }
...
async function processMessage(
  ...
  try {
    ...
    console.log("Debug - Stream created, waiting for response chunks...");
    ...
    for await (const chunk of stream) {
      if (chunk && typeof chunk === "object" && "agent" in chunk) {
        ...
        console.log("Debug - Received response chunk:", String(agentChunk.agent.messages[0].content));
      }
    }

    console.log("Debug - Final response:", response.trim());
    ...
    if (error?.message?.includes('API key')) {
      return "I'm having trouble with my API authentication. Please check the API key configuration.";
    }
...
async function handleMessage(message: DecodedMessage, client: Client) {
  ...
  try {
    ...
    console.log("Debug - Agent initialized for:", senderAddress);
    ...
    console.log(`Debug - Sent response to ${senderAddress}: ${response}`);
  } catch (error) {
...
async function startMessageListener(client: Client) {
  ...
  console.log("Debug - Client inbox ID:", client.inboxId);
  
  const stream = await client.conversations.streamAllMessages();
  console.log("Debug - Message stream initialized");
  
  for await (const message of stream) {
    console.log("Debug - Stream received something:", message ? "message" : "null");
    if (message) {
...
async function main(): Promise<void> {
  ...
  console.log("- WALLET_KEY:", WALLET_KEY ? "✓ Set" : "✗ Missing");
  console.log("- ENCRYPTION_KEY:", ENCRYPTION_KEY ? "✓ Set" : "✗ Missing");
  console.log("- NETWORK_ID:", NETWORK_ID);
  console.log("- OPENAI_API_KEY:", OPENAI_API_KEY ? "✓ Set" : "✗ Missing");
  console.log("- CDP_API_KEY_NAME:", CDP_API_KEY_NAME ? "✓ Set" : "✗ Missing");
  console.log("- CDP_API_KEY_PRIVATE_KEY:", CDP_API_KEY_PRIVATE_KEY ? "✓ Set" : "✗ Missing");
